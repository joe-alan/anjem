...
> **Date:** __________ (fill in when tested)
> **Tester:** __________
...
| Test                                                                                                   | Status        |
| ------------------------------------------------------------------------------------------------------ | ------------- |
| [A-1 Admin Login & Panel Access](#a-1-admin-login--panel-access)                                      | ⬜ Not tested |
| [A-2 Dashboard KPIs & Widgets](#a-2-dashboard-kpis--widgets)                                          | ⬜ Not tested |
| [A-3 Driver List — Filters & Columns](#a-3-driver-list--filters--columns)                             | ⬜ Not tested |
| [A-4 KYC Approve](#a-4-kyc-approve)                                                                   | ⬜ Not tested |
| [A-5 KYC Reject](#a-5-kyc-reject)                                                                     | ⬜ Not tested |
| [A-6 Grant Credits (Filament Action)](#a-6-grant-credits-filament-action)                              | ⬜ Not tested |
| [A-7 Deduct Credits (Filament Action)](#a-7-deduct-credits-filament-action)                           | ⬜ Not tested |
...
## A-17: Phase 1 API — KYC & Credits (direct)

> Tests the raw API endpoints from Phase 1 directly (Postman/curl), independent of the Filament UI layer.
...
curl -X POST http://localhost:8000/api/v1/auth/login \
  ...
  -d '{"email":"<admin_email>","password":"<admin_password>","as_admin":true}'
# Copy token from response

export TOKEN="<token>"
export DID="<driver_user_id>"
```

| #  | Endpoint                                          | Payload                                             | Expected                              | Result |
|----|---------------------------------------------------|-----------------------------------------------------|---------------------------------------|--------|
| 1  | `POST /api/v1/admin/drivers/{DID}/kyc/approve`     | `{}`                                                | 200, audit log created                |        |
| 2  | `POST /api/v1/admin/drivers/{DID}/kyc/reject`      | `{"reason":""}` (empty)                             | 422 validation error                  |        |
| 3  | `POST /api/v1/admin/drivers/{DID}/kyc/reject`      | `{"reason":"Documents too blurry."}`                | 200, is_verified=false                |        |
| 4  | `POST /api/v1/admin/drivers/{DID}/credits/grant`   | `{"amount":5,"reason":"Test grant"}`                | 200, balance+5                        |        |
| 5  | `POST /api/v1/admin/drivers/{DID}/credits/grant`   | `{"amount":101,"reason":"Over limit"}`              | 422 validation error (max 100)        |        |
| 6  | `POST /api/v1/admin/drivers/{DID}/credits/deduct`  | `{"amount":3,"reason":"Test deduct"}`               | 200, balance-3                        |        |
| 7  | `POST /api/v1/admin/drivers/{DID}/credits/deduct`  | `{"amount":9999,"reason":"Over balance"}`           | 422 insufficient credits              |        |
| 8  | `GET /api/v1/admin/drivers/{DID}/document`          | —                                                   | 200, ktm_url returned                 |        |
| 9  | `GET /api/v1/admin/audit-logs`                      | —                                                   | 200, paginated list                   |        |
| 10 | `GET /api/v1/admin/audit-logs?action_type=kyc_approve` | —                                               | 200, filtered results                 |        |
| 11 | Any endpoint with rider token                       | —                                                   | 403 Forbidden                         |        |
